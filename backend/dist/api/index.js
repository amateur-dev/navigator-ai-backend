...
async function logReferralEvent(db, referralId, event, type, description, user = 'system', details = null) {
    try {
...
app.post('/orchestrate', async (c) => {
    try {
        ...
        if (referralId !== 'unknown') {
            await logReferralEvent(db, referralId, 'Referral Created', 'system', 'Referral created through orchestration engine', 'system', {
                patientName,
                referralReason,
                specialty,
                insuranceProvider
...
app.post('/seed', async (c) => {
    ...
    `
        ...
        for (let i = 0; i < demoReferrals.length; i++) {
            ...
            if (refId) {
                await logReferralEvent(db, refId, 'Referral Created', 'system', `Referral created for ${ref.name} - ${ref.condition}`, 'system', { specialty: ref.specialty, payer: ref.payer });
                // Add status-specific logs
                if (status === 'Scheduled' || status === 'Completed') {
                    await logReferralEvent(db, refId, 'Eligibility Verified', 'eligibility', 'Insurance coverage confirmed', 'system');
                }
                if (status === 'Scheduled' || status === 'Completed') {
                    await logReferralEvent(db, refId, 'PA Request Submitted', 'pa', 'Prior authorization request submitted to payer', 'system');
                    await logReferralEvent(db, refId, 'PA Approved', 'pa', 'Prior authorization approved by payer', 'system');
                }
                if (status === 'Scheduled' || status === 'Completed') {
                    await logReferralEvent(db, refId, 'Appointment Scheduled', 'scheduling', 'Appointment scheduled with specialist', 'system');
                }
                if (status === 'Completed') {
                    await logReferralEvent(db, refId, 'Appointment Completed', 'scheduling', 'Patient attended scheduled appointment', 'system');
                }
            }
        }
