...
const extractionSchema = z.object({
  ...
  insuranceProvider: z.string().min(2, 'Insurance provider must be at least 2 characters'),
  referralReason: z.string().min(10, 'Referral reason must be at least 10 characters')
})
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        <h3 className="text-base font-medium text-default">Review Extracted Information</h3>
        <p className="text-sm text-muted-foreground">
          Please review and edit the extracted patient information before proceeding.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="grid gap-2">
          ...
          {errors.patientPhoneNumber && (
            <p className="text-xs text-red-500">{errors.patientPhoneNumber.message}</p>
          )}
        </div>
        ...
        <div className="grid gap-2">
          ...
          />
          {errors.patientEmail && (
            <p className="text-xs text-red-500">{errors.patientEmail.message}</p>
          )}
        </div>
        ...
        <div className="grid gap-2">
          ...
          {errors.referralReason && (
            <p className="text-xs text-red-500">{errors.referralReason.message}</p>
          )}
        </div>
      </div>

      <div className="flex gap-3">
        <Button type="submit" className="h-12 px-5 min-w-[240px]" disabled={isSubmitting}>
