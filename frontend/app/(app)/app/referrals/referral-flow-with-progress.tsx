  ...
  },
  {
    ...
    substeps: [
      ...
      },
      {
        ...
        status: 'pending',
        delay: 1000,
        dynamicValue: insuranceStatus ? `Status: ${insuranceStatus}` : undefined
      }
    ]
  },
  {
...
export const ReferralFlowWithProgress = () => {
  ...
  const [showOrchestrationProgress, setShowOrchestrationProgress] = React.useState(false)
  const [uploadProgressComplete, setUploadProgressComplete] = React.useState(false)
  const [orchestrationProgressComplete, setOrchestrationProgressComplete] = React.useState(false)
  const toastShownRef = React.useRef(false)

  const { uploadMutation, orchestrateMutation, confirmMutation, orchestrationData, resetAll } =
    useReferralUpload()
  ...
  const [orchestrationSteps, setOrchestrationSteps] = React.useState<ProgressStep[]>(
  ...
  React.useEffect(() => {
    ...
    if (foundDoctorStep?.status === 'completed' && !doctorMatchedToastShownRef.current) {
      // Use dynamicValue from step or fallback to orchestrationData
      const doctorName =
        foundDoctorStep.dynamicValue || orchestrationData?.data?.assignedDoctor || 'specialist'
  ...
  const isOrchestrating = orchestrateMutation.isPending || confirmMutation.isPending
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-auto px-4">
        ...
        {showUploadProgress && !uploadProgressComplete && uploadedFile && showUploadProgress && (
          <ReferralUploadLoading fileName={uploadedFile.file.name} />
        )}

        {/* Show loading during orchestration instead of form */}
        {showOrchestrationProgress && !orchestrationProgressComplete && uploadedFile && (
          <ReferralUploadLoading
        ...
        {isComplete && confirmMutation.data && orchestrationProgressComplete && (
          <ReferralConfirmation
